...
namespace smiBLL
{
    public class clCCR : clTableBase<clCCR>
    {
        ...
        public override void InsertEntity()
        {
            ...
            {
                Logger.LogError("Error inserting ccrs from database", ex);
                throw ex;
            }
        }
        ...
        public override void UpdateEntity()
        {
            try
            {
                using (DBsmiEntities DbContext = new DBsmiEntities(DBConnection.GetEFSMIDataBaseConStr()))
                {
                    ccr db_ccr = DbContext.ccrs.Where(cr => cr.id == this.id).FirstOrDefault();
                    db_ccr.idchild = this.idchild;
                    db_ccr.nyear = this.nyear;
                    db_ccr.nlivro = this.nlivro;
                    db_ccr.nr_consulta = this.nr_consulta;
            ...
            {
                Logger.LogError("Error updating ccrs from database", ex);
                throw ex;
            }
